znajdz_koncentracje_pozarow <- function(df, R = 300,
                                        nazwa_kolumny = "Netto",
                                        prog_suma = 1000) {
  ...
  # 1. Zamiana na sf i transformacja do metrów
  ...
  # 2. Sumy w okręgach (z uwzględnieniem punktu centralnego)
  for (i in seq_len(n)) {
    indeksy <- sasiedzi$id[[i]]
    
    # DODANE: dodajemy punkt centralny i
    indeksy <- unique(c(indeksy, i))
    
    lista_sasiadow[[i]] <- indeksy
    sumy[i] <- sum(as.numeric(df[[nazwa_kolumny]][indeksy]), na.rm = TRUE)
  }
  
  # 3. Sygnatury sąsiedztw (do usuwania duplikatów)
  ...
  # 4. Sortowanie i usunięcie duplikatów grup
  ...
  # 5. Filtr: tylko okręgi z sumą powyżej progu
  ...
  # 6. Budowanie listy grup z odległościami
  for (k in seq_len(nrow(wybrane_grupy))) {
    ...
    odleglosci <- sapply(indeksy_i, function(j) {
      sqrt((wsp_xy[j, 1] - srodek_xy[1])^2 +
             (wsp_xy[j, 2] - srodek_xy[2])^2)
    })
    ...
    df_grupa <- df_grupa[order(df_grupa$odleglosc), ]
