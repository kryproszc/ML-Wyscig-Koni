...
>(({ onSave, onCancel, onFinalSave, hasExistingCalculations }, ref) => {
  ...
  const handleClose = () => {
    if (hasChanges) {
      ...
      if (hasCalculations) {
        console.log('⚠️ [EditableIncurredTriangle] Są obliczenia w systemie - pokazuję ostrzeżenie o utracie obliczeń');
        setShowCalculationsWarning(true);
      } else {
        console.log('✅ [EditableIncurredTriangle] Zamykanie - brak zmian i obliczeń');
        if (onCancel) {
          onCancel();
        }
      }
    ...
    return (
      <Card>
        <CardContent className="p-4">
          <p className="text-gray-400">Brak danych trójkąta incurred do edycji. Najpierw wczytaj dane.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>✏️ Edycja trójkąta danych incurred</span>
